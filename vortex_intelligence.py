...
class VortexIntelligence:
    # ANSI Color Constants for Dashboard
    GREEN = "\033[92m"
    RED = "\033[91m"
    YELLOW = "\033[93m"
    CYAN = "\033[96m"
    ...
    def __init__(self, X, y, task='classification', imbalance_threshold=4, skew_threshold=0.75, kurtosis_threshold=10.0, outlier_iqr_multiplier=3.0, feature_names=None):
        ...
        # Categorical tracking placeholders
        self.cat_features = []
        self.cat_idx = []
    ...
    def get_report(self):
        ...
        # Initialize/Reset categorical tracking lists
        self.cat_features = []
        self.cat_idx = []

        for i, col in enumerate(self.X.columns):
            level = self._determine_data_level(col)
            # Identify Nominal and Ordinal as categorical for indexing and modeling
            if level in ["Nominal", "Ordinal"]:
                self.cat_features.append(col)
                self.cat_idx.append(i)
                # Cast for internal LGBM feature importance
                X_tmp[col] = X_tmp[col].astype('category')
        
        # Internal Predictive Importance Calculation
        ...
        for col in self.X.columns:
            ...
            if level in ["Interval", "Ratio"]:
                skew_val = self.X[col].skew()
                kurt_val = self.X[col].kurtosis()
