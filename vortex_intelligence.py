...
class VortexIntelligence:
    ...
    Now includes Feature-Level AUC-ROC Analysis.
    """
    ...
    def _generate_text_summary(self):
        ...
        num_stats = self.report[self.report['dtype'].str.contains('int|float|bool')]
        ...
        avg_auc = self.report['auc_roc'].mean() if self.task == 'classification' else 0
        ...
        if self.task == 'classification':
            ...
            top_feature = self.report.index[0]
            top_auc = self.report.iloc[0]['auc_roc']
            print(f"🛡️ Predictive: Top Feature [{top_feature}] has AUC-ROC of {top_auc:.4f}")
    ...
    def get_report(self):
        ...
        for col in num_cols:
            ...
            # AUC-ROC Logic (Feature Level)
            if self.task == 'classification' and self.y.nunique() == 2:
                # We use the absolute distance from 0.5 to show predictive strength regardless of direction
                try:
                ...
                stats.loc[col, 'auc_roc'] = np.nan

            stats.loc[col, 'abs_target_corr'] = self.X[col].corrwith(self.y).abs() if hasattr(self.X[col], 'corrwith') else np.nan

        stats['null_ratio'] = self.X.isnull().mean()
        stats['unique_counts'] = self.X.nunique()

        # Signal Detection via LGBM
        X_tmp = self.X.copy()
        for col in X_tmp.select_dtypes(exclude=[np.number]).columns:
            X_tmp[col] = X_tmp[col].astype('category')

        if self.task == 'classification':
            model = LGBMClassifier(n_estimators=100, importance_type='gain', verbosity=-1)
        else:
            model = LGBMRegressor(n_estimators=100, importance_type='gain', verbosity=-1)
        ...
        # Verdict Logic
        def judge(row):
            if row['lgbm_gain'] > 100 or row['auc_roc'] > 0.60:
